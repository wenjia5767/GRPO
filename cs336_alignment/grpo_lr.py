...
RUN_NAME = os.environ.get("RUN_NAME", "grpo_run")
...
if __name__ == "__main__":
    learning_rates_to_sweep = [1e-6] # <-- Define your LRs here
    final_results = {}

    for lr_sweep in learning_rates_to_sweep:
        print(f"\n==============================================")
        print(f"🚀 Starting sweep for learning rate: {lr_sweep}")
        print(f"==============================================\n")

        # --- Make run name and paths dynamic for each LR ---
        RUN_NAME = f"grpo_run_lr_{lr_sweep}"
        run_dir = os.path.abspath(f"./{RUN_NAME}")
        ...
        final_accuracy = eval_accs[-1] if eval_accs else 0.0
        final_results[lr_sweep] = final_accuracy
        print(f"✅ Sweep for LR {lr_sweep} finished. Final accuracy: {final_accuracy:.4f}")

    # --- Add this AFTER the loop to print the summary ---
    print("\n\n==============================================")
    print("✨ Learning Rate Sweep Summary ✨")
    print("==============================================")
    for lr, acc in final_results.items():
        print(f"Learning Rate: {lr:<8} | Final Accuracy: {acc:.4f}")